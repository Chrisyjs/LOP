<template>
  <div class="form-wrap">
    <div class="title">{{currentYear}} ST 报名表</div>
    ...
    <div class="form">
      ...
      >
        ...
        <div class="item-wrap">
          <div class="item-label form-required">您对{{currentYear}}ST的期待</div>
          <van-field
            :key="26"
            :rules="[{ required: true, message: '请输入相应内容' }]"
            class="border-bottom"
          ...
          >
            <template #input>
              <van-checkbox v-model="hasRead" shape="square">
                我已理解报名条件，确认报名信息无误，愿意报名{{currentYear}}ST
              </van-checkbox>
            </template>
          </van-field>
        </div>
      ...
      <van-dialog
        ...
        title="报名成功"
        :message="`感谢您报名${currentYear}ST，上帝纪念！`"
      ></van-dialog>
    </div>
  </div>
</template>
